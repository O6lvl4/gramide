...
- **`src/parser.almd`** — エンジン。文法は `Rule` 値（`Tok`, `Lit`, `Seq`, `Alt`, `Rep`, `Opt`,
  ...
  ノードが立ちます。括弧の釣り合わないファイルは、最初から回復モードで読みます。
...
括弧の釣り合わないファイルは、まず括弧を種類とインデントで対応づける。行頭の閉じ括弧は、同じ種類で
自分と同じ深さにインデントされた行の開き括弧と対になり、それより内側の、深い行にある開き括弧が
閉じ括弧を失ったものとわかる。その括弧は、自分の行より深くない次の行の手前で終わる。`}` が消えた
`if` の本体は `if` の次の文が始まるところで終わり、それを囲む関数は書かれたとおりに読める。
こうしたファイルは厳密なパースを一切しない。単独の括弧を読む文法はないからで、読み手は対にならなかった
括弧を名指しする(``21493:26: `{` is never closed``)。それでも item が失敗したら、recover site は
次の 3 つのうち `ERROR` の下に入るトークンが最も少ないものを取る。item 規則が次に読める位置まで
スキップする、失敗が最も遠くまで進んだ位置に閉じ括弧を 1 つあるものとして読み直す、ファイル末尾で
開いたままの本体を残す。Python はレイアウト段で、
